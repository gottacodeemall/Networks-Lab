...
int main(int argc,char** argv){
  ...
  float a[2];
  char buffer[1024];
  struct sockaddr_in serverAddr,serverAddrc;
  socklen_t addr_size,addr_sizec;
  ...
  connect(clientSocket, (struct sockaddr *) &serverAddr, addr_size);
    ...
    while(1){
        ...
        char service[1024];
    		...
    		scanf(" %s",to);
        printf("enter the email service \n");
        scanf(" %s",service);
        ...
        serverAddrc.sin_port = htons(atoi( argv[4]) );
        serverAddrc.sin_addr.s_addr = inet_addr(argv[3]);
        ...
        send(centralSocket,&service,sizeof(service),0);
        n=recv(centralSocket,&valid,sizeof(valid),0);
        close(centralSocket);
        if(valid==0){
          printf("The mail server is not registered with central server \n");
        }
        else{
          struct mail sending;
